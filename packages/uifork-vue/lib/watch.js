...
/**
 * ComponentManager - Manages a single Vue component's versions
 */
class ComponentManager {
  ...
  getVersionFiles() {
    ...
    // Vue version files: ComponentName.v1.vue, ComponentName.v1_2.vue
  ...
  detectFileExtension(_versionKey) {
    return ".vue";
  }
  ...
  generateVersionsFile() {
    ...
    const versions = versionFiles
      .map((file) => {
        ...
        const renderValue = this.generateImportName(file);
    ...
    if (this.lazy) {
      const lazyImports = versionFiles
        .map((file) => {
          ...
          const filePath = `./${file}`;
          return `const ${importName} = () => import("${filePath}");`;
        })
        .join("\n");
      imports = `// Lazy load components\n${lazyImports}`;
    } else {
  ...
  handleFileRename() {
    ...
    if (added.length > 0 || removed.length > 0) {
      ...
      if (removed.length === 1 && added.length === 1) {
        ...
        const oldMatch = oldFile.match(new RegExp(`^${this.componentName}\\.v([\\d_]+)\\.vue$`));
        const newMatch = newFile.match(new RegExp(`^${this.componentName}\\.v([\\d_]+)\\.vue$`));
  ...
  handleVersionsKeyChange() {
    ...
    if (added.length > 0 || removed.length > 0) {
      ...
      if (removed.length === 1 && added.length === 1) {
        ...
        const oldFileName = `${this.componentName}.v${oldVersion}.vue`;
        const newFileName = `${this.componentName}.v${newVersion}.vue`;
...
/**
 * VersionSync - Main class that manages multiple Vue components
 */
class VersionSync {
  constructor(watchPath, options = {}) {
    ...
    if (this.components.size === 0) {
      ...
      console.log("Run 'npx uifork-vue init <component-path>' to create one.");
    } else {
  ...
  startServer() {
    ...
    this.wss.on("connection", (ws) => {
      ...
      ws.send(
        JSON.stringify({
          type: "ack",
          payload: { message: "Connected to uifork-vue watch server" },
        }),
      );
...
`;
  ...
  handleRenameVersion(ws, payload) {
    ...
    try {
      ...
      const targetFilePath = manager.getVersionFilePath(newVersion);
      if (fs.existsSync(targetFilePath)) {
        throw new Error(`Target version already exists: ${newVersion}`);
      }

      const fileVersion = manager.versionKeyToFileVersion(newVersion);
      const finalTargetPath = path.join(
        manager.watchDir,
        `${manager.componentName}.v${fileVersion}.vue`,
      );
  ...
  handleFileChange(eventType, filePath) {
    ...
    for (const [name, manager] of this.components) {
      const versionPattern = new RegExp(`^${manager.componentName}\\.v([\\d_]+)\\.vue$`);
