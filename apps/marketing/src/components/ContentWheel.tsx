...
export type ContentWheelProps<T = ReactNode> = {
  ...
  renderItem?: (item: T, distance: number) => ReactNode
...
export function ContentWheel<T extends ReactNode>({
  ...
  const { stepRad, startDeg, spinDeg, zOffset } = useMemo(() => {
    ...
    const spinDeg =
      rotationMultiplier * activeIndex * degreesPerItem + offsetDeg
  ...
  const normalizedActiveIndex =
    ((Math.floor(activeIndex) % itemCount) + itemCount) % itemCount

  const containerSize = sizes[normalizedActiveIndex] ?? { width: 0, height: 0 }

  const defaultRenderItem = useCallback(
    (value: T, distance: number) => {
      return (
        <div
          style={{
  ...
  const renderAtDistance = (iOriginal: number, distance: number) =>
    renderItem
      ? renderItem(items[iOriginal], distance)
      : defaultRenderItem(items[iOriginal], distance)
          ...
          >
            ...
            >
              {items.map((_, itemIndex) => {
                ...
                const thetaDeg =
                  startDeg + indexOnWheel * ((stepRad * 180) / Math.PI)
