...
class ComponentManager {
  ...
  getVersionFiles() {
    ...
    if (matchedFiles.length === 0) {
      ...
      console.log(
        `[${this.componentName}] Files in directory: ${files.filter((f) => f.includes(this.componentName)).join(", ") || "(none with component name)"}`,
      );
    }
  ...
  generateVersionsFile() {
    ...
    const content = `export const VERSIONS = {
${versions},
}
`;
...
class VersionSync {
  constructor(watchPath) {
    ...
    if (this.components.size === 0) {
      ...
      console.log("Run 'uifork init <component-path>' to create one.");
    } else {
...
`;
  ...
  handleFileChange(eventType, filePath) {
    ...
    if (eventType === "add" && filename.endsWith(".versions.ts")) {
      const manager = new ComponentManager(filePath);
